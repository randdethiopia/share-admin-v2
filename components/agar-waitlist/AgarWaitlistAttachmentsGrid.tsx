...
function getAttachmentType(url: string): AgarWaitlistAttachment["type"] {
	return isImageUrl(url) ? "image" : "document";
}

function getAttachments(application: AgarWaitlistApplication): AgarWaitlistAttachment[] {
	const items: AgarWaitlistAttachment[] = [];

	if (application.photosOfBusiness) {
		items.push({
			label: "Business photos",
			url: application.photosOfBusiness,
			type: getAttachmentType(application.photosOfBusiness),
		});
	}

	if (application.pitchDeck) {
		items.push({
			label: "Pitch deck",
			url: application.pitchDeck,
			type: getAttachmentType(application.pitchDeck),
		});
	}

	if (application.registrationCertificate) {
		items.push({
			label: "Registration certificate",
			url: application.registrationCertificate,
			type: getAttachmentType(application.registrationCertificate),
		});
	}
...
export function AgarWaitlistAttachmentsGrid({ application }: Props) {
		...
		<section className="space-y-4 rounded-2xl border border-slate-100 bg-white p-5 sm:p-6">
			<div>
				...
				<p className="mt-1 text-sm text-gray-500">
					Uploaded business photos and documents
				</p>
			</div>
