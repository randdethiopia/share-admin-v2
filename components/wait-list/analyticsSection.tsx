							...
							return (
								...
								>
									{fields.map((key) => (
										<td
											key={key}
											className="px-4 py-3 text-slate-600 whitespace-nowrap max-w-50 truncate"
		...
		<ScrollArea className="h-full">
			<div className="px-6 py-5 space-y-2">
				{stats.map((s) => (
					...
					>
						<p className="text-xs font-medium text-slate-700 min-w-35 truncate">{s.label}</p>
		...
		<div className="space-y-8">
			...
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-2">
				<div className="space-y-0.5">
					...
					<p className="text-sm text-slate-500">Analytics overview and reporting</p>
				</div>
			...
			<Modal open={customReportOpen} onClose={() => setCustomReportOpen(false)}>
				<div
					className="relative w-full max-w-5xl rounded-3xl border border-slate-200/60 bg-white shadow-2xl flex flex-col overflow-hidden"
					style={{
				...
				>
					{/* ── Modal Header ── */}
					<div className="flex items-center gap-4 px-7 py-5 border-b border-slate-100 shrink-0 bg-linear-to-r from-slate-50 to-white">
						...
						<div className="min-w-0">
							...
							<p className="text-[12px] text-slate-500 leading-tight mt-1">
								1) Choose fields  2) Preview  3) Export
							</p>
						</div>
						...
						<button
							...
							className="ml-2 flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition-colors"
							aria-label="Close"
						>
							<X className="h-4 w-4" />
						</button>
					...
					{/* ── Two-Panel Body ── */}
					<div className="flex flex-1 min-h-0 overflow-hidden flex-col lg:flex-row bg-slate-50/70">
						{/* LEFT — Field selector */}
						<div className="border-b lg:border-b-0 lg:border-r border-slate-100 flex flex-col overflow-hidden shrink-0 w-full lg:w-80 max-h-72 lg:max-h-none bg-white min-h-0">
							<FieldSelector
